...
function prikazTO(num, index) {
  return num  + `<span  class="brisac" id=${index} onclick="obrisiTO(this.id)"><i class="fa fa-close"></i></span>`;
}
function prikazCC(num, index) {
  return num  + `<span  class="brisac" id=${index} onclick="obrisiCC(this.id)"><i class="fa fa-close"></i></span>`;
}
function prikazBCC(num, index) {
  return num  + `<span  class="brisac" id=${index} onclick="obrisiBCC(this.id)"><i class="fa fa-close"></i></span>`;
}
...
function prikazSnimljenog() { 
  ...
  if(!x || x.length === 0 ){
    saves_n.innerHTML = '<small><p>Nemate nijednu snimljenu postavku!</small></p>';
    izbacivanjeStorage();
    mailAdressTo = [];
    mailAdressCc = [];
    mailAdressBcc = [];
  ...
  snimljeno = x;  
  uuu = snimljeno.map((num,index) => {return `<button class="${num}" id="${index}" onclick="selektuj(this.className,this.id)">${num}</button>`});
...
function deleteItem() {
  if(naziv_Pripreme.value == ''){
    alert("Morate selektovati postavku za brisanje")
  }
  else{
    const i = clickName;
    const u = clickID;
